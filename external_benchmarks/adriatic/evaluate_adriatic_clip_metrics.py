...
from external_benchmarks.adriatic.common import read_csv_rows, write_csv, write_json  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate clip-level whistle/noise metrics on the prepared Adriatic benchmark.",
...
def main() -> None:
    ...
    output_dir = (
        ...
        if args.output_dir
        else inference_dir / "_adriatic_clip_metrics"
    )
    ...
    for threshold in clip_thresholds:
        preds = np.asarray(
            [1 if int(row["detection_count"]) >= threshold else 0 for row in clip_rows],
            dtype=np.int64,
        )
            ...
            per_clip_rows.append(
                {
                    ...
                    "whistle_tag": row["whistle_tag"],
                    "whistle_quality": row["whistle_quality"],
                    "onset": row["onset"],
                    "offset": row["offset"],
                }
            )
    ...
    if not positive_df.empty:
        positive_df["whistle_quality"] = positive_df["whistle_quality"].replace("", "unknown")
        group_specs = [
            ("all_positives", positive_df),
            *(
                (f"tag_{tag}", group.copy())
                for tag, group in positive_df.groupby("whistle_tag", dropna=False)
            ),
            *(
                (f"quality_{quality}", group.copy())
                for quality, group in positive_df.groupby("whistle_quality", dropna=False)
            ),
        ]
    ...
    write_csv(
        ...
        positive_group_rows,
        list(positive_group_rows[0].keys()) if positive_group_rows else ["group_name", "clip_count"],
    )

    best_row = max(summary_rows, key=lambda row: (float(row["f1"]), float(row["balanced_accuracy"])))
    summary_payload = {
