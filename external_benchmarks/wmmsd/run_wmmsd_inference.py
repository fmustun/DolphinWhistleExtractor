...
from external_benchmarks.wmmsd.common import REPO_ROOT, read_csv_rows, write_json
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the current VGG16 whistle detector on a prepared WMMSD flat staging "
            "directory produced by prepare_wmmsd_manifest.py."
        ),
    ...
    parser.add_argument(
        "--output-dir",
        default="",
        help="Inference output directory. Defaults to <prepared-dir>/inference.",
    )
    parser.add_argument(
        "--threshold",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Inference batch size in 0.4 s windows.",
    )
    parser.add_argument(
        "--max-workers",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=96_000,
        help="Resample target rate used at inference time.",
    )
    parser.add_argument(
        "--start-time",
        type=float,
        default=0.0,
        help="Optional clip trimming start time in seconds.",
    )
    parser.add_argument(
        "--end-time",
        type=float,
        default=None,
        help="Optional clip trimming end time in seconds.",
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=0,
        help="Optional cap on the number of staged files to process.",
    )
    parser.add_argument(
        "--cpu",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Save spectrogram images of positive detections.",
    )
...
def main() -> None:
    ...
    print(
        "Next step       : python external_benchmarks/wmmsd/"
        "evaluate_wmmsd_positive_only.py "
