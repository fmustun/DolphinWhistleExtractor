...
from external_benchmarks.wmmsd.ultimate_suite import ULTIMATE_SCENARIOS


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate a single WMMSD inference run under multiple benchmark scenarios "
...
def main() -> None:
    ...
    output_dir = (
        ...
        if args.output_dir
        else inference_dir / "_wmmsd_ultimate_suite"
    )
    ...
    for scenario in ULTIMATE_SCENARIOS:
        ...
        scenario_status = (
            "ok"
            if present_positive_species and present_negative_species
            else "incomplete"
        )
        ...
        for threshold in clip_thresholds:
            labels = np.asarray(
                [
                    1 if str(row["species"]) in positive_species else 0
                    for row in scenario_dataset
                ],
                dtype=np.int64,
            )
            preds = np.asarray(
    ...
    complete_rows = [
        row for row in scenario_summary_rows if str(row["status"]) == "ok"
    ]
    best_by_f1 = max(
        complete_rows,
    ...
    if hardest_best:
        print(
            "Best hardest-case   : "
            f"{hardest_best['scenario']} @ min_det={hardest_best['clip_positive_min_detections']} "
            f"F1={float(hardest_best['f1']):.4f} "
            f"BA={float(hardest_best['balanced_accuracy']):.4f}"
        )
